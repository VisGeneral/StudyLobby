...
def profile():
    ...
    if text == "Kz2002890":
        ...
        return render_template("profile_lock.html")

@views.route('/profile')
def profilelock():
    return render_template('profile_lock.html')

@views.route('/notes/ru')
def notes_ru():
    return render_template('Russian_notes.html')
@views.route('/app')
def app():
...
def changelogs():
    changeloglist=[
        ...
        "October 28, 2022 (5:40 PM): Changed non signed in users being redirected to the login page and not the signup page",
        "October 28, 2022 (7:59 PM): Fixed an error with 7th grade math"
        ]
    user_agent = request.headers.get('User-Agent')
    user_agent = user_agent.lower()
    if "iphone" in user_agent:
...
@login_required
def studylobbymath7():
    questions={'1a': '½×⁵⁄₄=', '1b': '5/8', '2a': '¼×⁵⁄₃=', '2b': '5/12', '3a': 'Leave as an improper fraction: ¹⁰⁄₃×¹¹⁄₆=', '3b':'55/9', '4a': '⅙×⁸⁄₁₁=', '4b': '4/33', '5a': 'Leave as an improper fraction: ¹¹⁄₂÷½=', '5b': '11/4'}
    number = (random.randint(1, 5))
    answernum = (f'{number}b')
    questionnum = (f'{number}a')
    question = (questions.get(questionnum))
